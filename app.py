...
def main():
    ...
    st.sidebar.markdown('* nilson.cunhan@gmail.com \n'
                        '* [LinkedIn](https://www.linkedin.com/in/nilsoncunhan/) - '
                        '[Portfólio](https://nilsoncunha.github.io/portfolioweb/) - '
                        '[GitHub](https://github.com/nilsoncunha/portfolio)')

    if option == 'Análise':
        train = load_train()
        test = load_test()

        st.image('https://raw.githubusercontent.com/nilsoncunha/portfolioweb/master/assets/img/posts/enem.jpg',
                 use_column_width=True)
        st.title('Prevendo as notas de matemática do ENEM do ano de **2016**')

        # Minha apresentação
        st.markdown(
            '>*Análise baseada em uma das atividades propostas pelo programa de aceleração da **Codenation** que '
            'participei no final de 2019, **Acelera Dev - Data Science**, em Belo Horizonte.*')
        st.markdown(
        ...
        st.markdown("Conseguimos ver que o estado de São Paulo teve o maior número de candidados, seguido por Ceará e "
        ...
        st.markdown("Fazendo a verificação por sexo, conseguimos observar que as mulheres tiveram uma maior "
                    "participação na prova. *Nos gráficos, imagens, etc., o "
                    "[Streamlit](https://docs.streamlit.io/index.html) nos dá a opção de expandi-lo, colocando o "
                    "ponteiro do mouse em cima é exibido uma seta no canto superior direito*")
        sns.catplot(x='SG_UF_RESIDENCIA', col='TP_SEXO', kind='count', height=6, aspect=1.2, data=train)
        st.pyplot()

        st.markdown("Observamos abaixo a distribuição de idade dos participantes. *Apenas adicionamos 'st.pyplot()' "
                    "depois de montarmos o gráfico*")
        ...
        train['NU_NOTA_PROVAS'] = (train['NU_NOTA_CH'] + train['NU_NOTA_CN'] + train['NU_NOTA_LC'] + train[
            'NU_NOTA_MT']) / 4

        st.markdown(
            'Na redação temos alguns pontos que são observados no caso de fugir ao tema, for anulada, entre outros. '
        ...
        st.markdown('Visualizando agora as notas das provas por estado. *Utilizando o plotly novamente que fica muito '
        ...
        with st.echo():
            train_df.loc[train_df['TP_PRESENCA_CH'] != 1, 'NU_NOTA_CH'] = train_df.loc[
                train_df['TP_PRESENCA_CH'] != 1, 'NU_NOTA_CH'].fillna(0)
            train_df.loc[train_df['TP_PRESENCA_CN'] != 1, 'NU_NOTA_CN'] = train_df.loc[
                train_df['TP_PRESENCA_CN'] != 1, 'NU_NOTA_CN'].fillna(0)
            train_df.loc[train_df['TP_PRESENCA_MT'] != 1, 'NU_NOTA_MT'] = train_df.loc[
                train_df['TP_PRESENCA_MT'] != 1, 'NU_NOTA_MT'].fillna(0)
        ...
        train_df.loc[train_df['TP_PRESENCA_MT'] != 1, 'NU_NOTA_MT'] = train_df.loc[
            train_df['TP_PRESENCA_MT'] != 1, 'NU_NOTA_MT'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_LC'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_LC'].fillna(0)

        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_REDACAO'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_REDACAO'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP1'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP1'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP2'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP2'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP3'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP3'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP4'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP4'].fillna(0)
        train_df.loc[train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP5'] = train_df.loc[
            train_df['TP_PRESENCA_LC'] != 1, 'NU_NOTA_COMP5'].fillna(0)
        ...
        # train_df.drop(['SG_UF_RESIDENCIA', 'TP_SEXO', 'TP_COR_RACA', 'TP_NACIONALIDADE', 'TP_ST_CONCLUSAO',
        #               'TP_ANO_CONCLUIU', 'TP_LINGUA', 'TP_STATUS_REDACAO'], axis=1, inplace=True)

        # Features que foram removidas para utilizar no modelo de deploy
        train_df.drop(
            ['SG_UF_RESIDENCIA', 'TP_SEXO', 'TP_COR_RACA', 'TP_NACIONALIDADE', 'TP_ST_CONCLUSAO', 'TP_ANO_CONCLUIU',
        ...
        treino_op = ('Não', 'Sim')
        treino = st.selectbox(
            'Fez somente para treino?', index=0, key='treino', options=list(range(len(treino_op))),
            format_func=lambda x: treino_op[x]
        )

        st.subheader('Presença e Nota nas provas')

        st.markdown('Ciências da Natureza')
        pr_prova_cn_op = ('Não', 'Sim')
        pr_prova_cn = st.selectbox(
            'Presença', index=1, key='pr_prova_cn', options=list(range(len(pr_prova_cn_op))),
            format_func=lambda x: pr_prova_cn_op[x]
        )

        if pr_prova_cn == 0:  # se não estiver presente, a nota será 0
            nt_cn = st.number_input('Nota', value=0.0, min_value=0.0, max_value=0.0, key='nt_cn')
        else:
            nt_cn = st.number_input('Nota', min_value=0.0, max_value=1000.0, value=0.0, step=5.0, key='nt_cn')

        st.markdown('---')

        st.markdown('Ciências Humanas')
        pr_prova_ch_op = ('Não', 'Sim')
        pr_prova_ch = st.selectbox(
            'Presença', index=1, key='pr_prova_ch', options=list(range(len(pr_prova_ch_op))),
            format_func=lambda x: pr_prova_ch_op[x]
        )

        if pr_prova_ch == 0:  # se não estiver presente, a nota será 0
            nt_ch = st.number_input('Nota', value=0.0, min_value=0.0, max_value=0.0, key='nt_ch')
        else:
            nt_ch = st.number_input('Nota', min_value=0.0, max_value=1000.0, value=0.0, step=5.0, key='nt_ch')

        st.markdown('---')

        st.markdown('Linguagem e Códigos')
        pr_prova_lc_op = ('Não', 'Sim')
        pr_prova_lc = st.selectbox(
            'Presença', index=1, key='pr_prova_lc', options=list(range(len(pr_prova_lc_op))),
            format_func=lambda x: pr_prova_lc_op[x]
        )

        if pr_prova_lc == 0:  # se não estiver presente, a nota será 0
            nt_lc = st.number_input('Nota prova', value=0.0, min_value=0.0, max_value=0.0, key='nt_lc')
            nt_redacao = st.number_input('Nota redação', value=0.0, min_value=0.0, max_value=0.0, key='nt_redacao')
        else:
            nt_lc = st.number_input('Nota prova', min_value=0.0, max_value=1000.0, value=0.0, step=5.0, key='nt_lc')
            nt_redacao = st.number_input('Nota redação', min_value=0.0, max_value=1000.0, value=0.0, step=5.0,
                                         key='nt_redacao')
        ...
        if st.button('Fazer previsão:'):
            int_features = [estados, idade, escola, treino, pr_prova_cn, pr_prova_ch, pr_prova_lc,
                            nt_cn, nt_ch, nt_lc, nt_redacao]
        ...
        st.markdown(
            'Uma pessoa que gostou de trabalhar com dados e viu que pode ser gerado muito valor através deles. '
            'Foi com esse intuito que comecei a fazer cursos e iniciar a pós-graduação em Ciência de Dados e Big Data.')
